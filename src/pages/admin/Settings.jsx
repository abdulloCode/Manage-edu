...
import { User, Mail, Phone, Save, Palette, Loader2 } from 'lucide-react'
...
export default function SettingsPage() {
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {message && (
        <div className={`alert py-2 text-sm ${message.type === 'success' ? 'alert-success' : 'alert-error'}`}>
          <span>{message.text}</span>
        </div>
      )}

      <ProfileTab onMessage={showMessage} />
    </div>
  )
}
...
function ProfileTab({ onMessage }) {
  ...
  if (fetchLoading) {
    return (
      <div className="bg-base-100 rounded-xl border border-base-200 p-6 flex items-center justify-center min-h-[200px]">
  ...
  return (
    <div className="bg-base-100 rounded-xl border border-base-200 p-6">
      <h2 className="text-lg font-semibold text-base-content mb-6">Profile Settings</h2>
